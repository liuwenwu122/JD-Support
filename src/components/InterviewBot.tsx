...
import { createInterviewSession, generateInterviewFeedback, AIChatSession } from '../services/aiService';
import { MessageSquare, Send, User, Bot, Mic, MicOff, Play, Square, CheckCircle, AlertTriangle, RefreshCcw, StopCircle } from 'lucide-react';
...
const InterviewBot: React.FC<InterviewBotProps> = ({ targetRole, language, jobDescription, settings }) => {
  ...
  const endInterview = async () => {
    ...
    try {
      const feedback = await generateInterviewFeedback(messages, language, settings);
        ...
        {messages.map((msg) => (
          ...
          >
            <div className={`flex max - w - [80 %] gap - 3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'} `}>
              <div className={`w - 8 h - 8 rounded - full flex - shrink - 0 flex items - center justify - center ${msg.role === 'user' ? 'bg-blue-600' : 'bg-green-600'
      ...
      <div className="p-4 bg-white border-t border-gray-200">
        <div className="relative flex items-center">
          ...
          <button
            ...
            className={`absolute right - 2 p - 2 rounded - full transition - colors ${input.trim() ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-gray-200 text-gray-400'
              } `}
          >
            <Send size={20} />
          </button>
