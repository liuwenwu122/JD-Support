...
const MainLayout: React.FC = () => {
    ...
    const handleAutoTag = async () => {
        ...
        setIsTagging(true);
        try {
            const tags = await generateTags(resumeData, language, settings);
            ...
            showNotification("Tagging failed", 'error');
        } finally {
            setIsTagging(false);
        }
    };
    ...
    if (!activeProfileId || !resumeData) {
        return (
            <div className="min-h-screen bg-gray-50 flex flex-col">
                ...
                {/* Mobile Menu for Dashboard View */}
                <MobileMenu
                    isOpen={showMobileMenu}
                    onClose={() => setShowMobileMenu(false)}
                    onSave={() => { }} // Not available in dashboard
                    onHistory={() => { }} // Not available in dashboard
                    onAutoTag={() => { }} // Not available in dashboard
                    onLanguage={() => setLanguage(l => l === 'zh' ? 'en' : 'zh')}
                    onSignOut={signOut}
                    onOpenSettings={() => setShowSettingsModal(true)}
                    language={language}
                    isTagging={false}
    ...
    return (
        <div className="flex flex-col h-screen bg-gray-100 overflow-hidden font-sans text-slate-800 app-container">
            ...
            {/* Top Navigation */}
            <nav className="bg-white border-b border-gray-200 h-16 shrink-0 flex items-center justify-between px-4 md:px-6 shadow-sm z-30 relative no-print">
                <div className="flex items-center gap-2 md:gap-4 flex-1 min-w-0">
                    ...
                    <div className="flex flex-col min-w-0 flex-1">
                        <h1 className="font-bold text-sm text-slate-800 truncate">{activeProfile?.name}</h1>
                        <div
                            className="flex items-center gap-1 text-xs text-blue-600 cursor-pointer hover:underline min-w-0"
                            onClick={() => setShowTargetModal(true)}
                        >
                            <Target size={12} className="flex-shrink-0" />
                            <span className="truncate">{activeProfile?.targetRole}</span>
                            <PenLine size={10} className="flex-shrink-0" />
                        </div>
                    </div>

                    {/* Desktop Tabs */}
                    <div className="hidden md:flex h-6 w-px bg-gray-200 mx-2"></div>
                ...
                <button
                    ...
                    className="md:hidden p-2 hover:bg-gray-100 rounded-full text-gray-500 transition-colors flex-shrink-0"
                >
                    <Menu size={24} />
                </button>
            </nav>
            ...
            <div className="flex flex-1 overflow-hidden relative">
                {/* Main Content Area */}
                <main className={`flex-1 overflow-y-auto p-2 md:p-8 pb-20 md:pb-4 transition-all duration-300 ${showHistory ? 'md:mr-80' : ''} main-content`}>
                ...
                <div className={`absolute top-0 right-0 bottom-0 w-full md:w-80 bg-white shadow-xl transform transition-transform duration-300 z-40 border-l border-gray-200 ${showHistory ? 'translate-x-0' : 'translate-x-full'} no-print`}>
                    {activeProfile && activeVersionId && (
                        <VersionHistory
