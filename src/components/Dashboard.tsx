...
const Dashboard: React.FC<DashboardProps> = ({ profiles, onSelect, onCreate, onImport, onDelete, language, settings }) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = async () => {
      ...
      try {
        const parsedData = await parseResumeFile(base64String, file.type, settings);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 animate-in fade-in duration-500 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          ...
          <button
            ...
            className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-white border border-gray-200 text-slate-700 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all shadow-sm font-semibold"
            disabled={isParsing}
          >
            <FileUp size={18} />
            {language === 'zh' ? '导入 PDF' : 'Import'}
          </button>
          <button
            onClick={handleStartCreate}
            className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all shadow-md hover:shadow-lg font-semibold"
            disabled={isParsing}
          >
            <Plus size={18} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-fr">
        ...
        <div
          onClick={handleStartCreate}
          className="border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center text-gray-400 hover:border-blue-400 hover:text-blue-500 hover:bg-blue-50/10 cursor-pointer min-h-[280px] transition-all group"
        >
          <div className="p-4 bg-gray-50 rounded-full mb-4 group-hover:bg-blue-100 transition-colors">
            <Plus size={32} className="text-gray-400 group-hover:text-blue-600" />
          </div>
          <span className="font-medium text-lg">
            {language === 'zh' ? '创建新简历' : 'Create New Resume'}
          </span>
        </div>
        ...
        {profiles.map((profile) => (
          <div
            key={profile.id}
            className="group bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-lg hover:border-blue-300 transition-all duration-300 overflow-hidden flex flex-col min-h-[280px] relative"
          >
            <div className="p-6 flex-1 cursor-pointer flex flex-col" onClick={() => onSelect(profile.id)}>
              ...
              <h3 className="text-xl font-bold text-slate-800 mb-2 truncate group-hover:text-blue-600 transition-colors" title={profile.name}>
                {profile.name}
              </h3>

              <div className="flex items-center gap-2 text-sm text-slate-700 font-medium mb-auto">
                <Target size={16} className="text-blue-500 flex-shrink-0" />
                <span className="truncate max-w-[200px]" title={profile.targetRole}>{profile.targetRole || 'General'}</span>
              </div>
            ...
            <div className="border-t border-gray-100 px-6 py-3 bg-gray-50 flex justify-between items-center mt-auto">
