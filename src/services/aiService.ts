...
const callOpenAI = async (settings: AISettings, messages: any[]) => {
  ...
  const body: any = {
    ...
    messages: messages,
    stream: false,
    max_tokens: 4096, // Increased for resume analysis
    temperature: 0.7,
    top_p: 0.7,
    top_k: 50,
  ...
  const timeoutId = setTimeout(() => controller.abort(), 180000); // 180s timeout (3 mins) for reasoning models
...
export const parseResumeFile = async (base64Data: string, mimeType: string, settings?: AISettings): Promise<ResumeData> => {
  ...
  if (provider === 'openai') {
    ...
    const jsonStr = await callOpenAI(settings!, messages);
...
export const generateTags = async (resume: ResumeData, lang: Language, settings?: AISettings): Promise<string[]> => {
  ...
  try {
    if (settings?.provider === 'openai') {
      ...
      const res = await callOpenAI(settings, messages);
...
export const createInterviewSession = (targetRole: string, lang: Language, jobDescription?: string, settings?: AISettings) => {
  if (settings?.provider === 'openai') {
